...
//TODO: add this onto play right now!
public class MinesDeeper extends Applet implements MouseListener {
	...
	int row = 10, col = 10, numbombs = 10, Setup = 3; // number of rows columns,
														// and bombs
		...
		for (int x = 0; x < row; x++) {
			for (int y = 0; y < col; y++) {
				if (e.getSource() == table[x][y]) {
					if (e.getButton() == e.BUTTON1 && flag[x][y] == false) // if
																			// left
																			// click,
																			// and
																			// there
																			...
																			// button
					{
						if (bomb[x][y] == true) // if you you click on a bomb,
						...
						exposed[x][y] = true;
						checkwinbool[x][y] = true; // these set to true mean
													// that the button has been
													// clicked
						surbombs = Integer.toString(surroundingBombs(x, y)); // gets
																				// the
																				// number
																				// of
																				// surrounding
																				...
																				// to
																				// setLabel
						table[x][y].setLabel(surbombs); // sets the label to be
														// the number of bombs
														// in the 8 surrounding
														// buttons
						if (surroundingBombs(x, y) == 0) {
							check(x, y); // calls a recursive method so that if
											// a "0" is there the surrounding 8
											// buttins must be exposed and if
											// one of those is "0" it too must
											// be exposed and so on
						}
					} else if (e.getButton() == e.BUTTON3) // if it is a right
															// click
					{
						if (flag[x][y] == true) // if there is a flag already
												// present set it so that there
												// is no flag
						{
							table[x][y].setLabel("");
							table[x][y].setForeground(Color.white);
							...
							checkwinbool[x][y] = false;
							bombsremaining++;
						} else if (checkwinbool[x][y] == false || bomb[x][y] == true) // if
																						// there
																						// is
																						// no
																						// flag,
						...
						Bombs.setText(Integer.toString(bombsremaining));
					...
					} else if (e.getButton() == e.BUTTON2 && flag[x][y] == false && checkwinbool[x][y] == true && bomb[x][y] == false) // if
																																		// both
																																		// left
																																		// and
																																		// right
																																		...
																																		// same
																																		// time
					{ // only executes if there is no flag, it has been exposed,
						// and no bomb
						if (flags(x, y) == surroundingBombs(x, y)) // checks
																	// that the
																	// number of
																	// flags
																	// around
																	...
																	// [x][y]
						{
							for (int q = x - 1; q <= x + 1; q++) {
								for (int w = y - 1; w <= y + 1; w++) {
									if (q < 0 || w < 0 || q >= row || w >= col) // makes
																				...
																				// wall
										break;
									if (bomb[q][w] == false && flag[q][w] == true) // if
																					// there
																					// is
																					// no
																					// bomb,
							...
							if (gameover == false) {
								expose(x, y); // eposes the surrounding 8
												// buttons
								check(x, y); // checks if any of those are "0"
												// and calls the recursive
												// method
							}
						}
					}
					if (gameover == false) // this just calls the method for
											// changing the colours of the
											// buttons if they have been clicked
						clicked();
				}
	...
	public static void main(String[] args){
		...
		MinesDeeper md = new MinesDeeper();
		md.init();
		tF.setContentPane(md);
		tF.setBounds(300, 200, 700, 400);
		tF.setVisible(true);
	}
}